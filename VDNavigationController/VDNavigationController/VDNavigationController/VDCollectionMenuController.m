...
- (void)updateViewConstraints {
    ...
    CGFloat collectionViewHeightOffset = 0.0f;
    
    // Configure Segmented Controller if necessary
    if ([self displaySegmentedControl]) {
        
        UIView *viewAboveCollectionView = self.segmentedButtons[0];
        ...
        [self.segmentedButtons enumerateObjectsUsingBlock:^(UIButton *currentButton, NSUInteger idx, BOOL *stop) {
            ...
            [weakSelf.view addConstraint:[NSLayoutConstraint constraintWithItem:currentButton attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:weakSelf.view attribute:NSLayoutAttributeTop multiplier:1.0f constant:0.0f]];
